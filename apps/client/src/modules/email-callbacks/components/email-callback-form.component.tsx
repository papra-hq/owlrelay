...
export const EmailCallbackForm: Component<{
  ...
  const { form, Field, Form, createFormError, FieldArray } = createForm({
    schema: v.object({
      ...
      domain: v.pipe(v.picklist(availableDomains, t('email-callbacks.form.validation.domain.invalid'))),
      webhookUrl: v.pipe(v.string(), v.nonEmpty(t('email-callbacks.form.validation.webhook-url.required')), v.url(t('email-callbacks.form.validation.webhook-url.invalid'))),
      webhookSecret: v.union([
        v.pipe(v.string(), v.minLength(16, t('email-callbacks.form.validation.webhook-secret.min-length'))),
        // When not set, the secret is an empty string
        v.literal(''),
      ]),
      allowedOrigins: v.optional(v.array(v.pipe(v.string(), v.rfcEmail(t('email-callbacks.form.validation.allowed-origins.invalid'))))),
    }),
  ...
  return (
    <Form class="flex flex-col gap-4">
      <div class="bg-card sm:rounded-lg p-6 border-y sm:border">
        <div class="flex items-center justify-between gap-2  mb-4">
          <div>
            ...
            <p class="text-sm text-muted-foreground">{t('email-callbacks.form.address.description')}</p>
          </div>
          {(props.showRandomAddressButton ?? true) && (
            <div>
              <Button variant="outline" onClick={() => setValue(form, 'username', generateId())} class="flex-shrink-0 gap-2 hidden sm:flex whitespace-nowrap">
                <div class="i-tabler-refresh size-4 flex-shrink-0"></div>
                {t('email-callbacks.form.address.random-address')}
              </Button>

              <Button variant="outline" onClick={() => setValue(form, 'username', generateId())} class="flex-shrink-0 gap-2 sm:hidden" size="icon">
                  ...
                  <Field name="domain">
                    {field => (
                      <TextFieldRoot>
                        ...
                        >
                          <SelectTrigger class="border-none shadow-none  text-muted-foreground px-0 py-1 h-auto">
      ...
      <div class="bg-card sm:rounded-lg p-6 border-y sm:border">
        <div class="flex items-center justify-between gap-2  mb-4">
          <div>
            ...
            <p class="text-sm text-muted-foreground">{t('email-callbacks.form.webhook.description')}</p>
          </div>
        </div>
        ...
        <Field name="webhookUrl">
          {(field, inputProps) => (
            <TextFieldRoot class="flex flex-col gap-1 mt-6">
              ...
              <TextField type="text" id="webhookUrl" placeholder={t('email-callbacks.form.webhook.url.placeholder')} {...inputProps} value={field.value} aria-invalid={Boolean(field.error)} />
              {field.error && <div class="text-red-500 text-sm">{field.error}</div>}
            </TextFieldRoot>
          )}
        </Field>
          ...
          {(field, inputProps) => (
            <TextFieldRoot class="flex flex-col gap-1 mt-6">
              <TextFieldLabel for="webhookSecret" class="flex items-baseline gap-2">
                ...
                <span class="bg-background text-xs leading-tight px-1.5 py-0.5 rounded border text-muted-foreground">{t('email-callbacks.form.recommended')}</span>
              </TextFieldLabel>

              <div class="flex items-center gap-2">
                <TextField type="text" id="webhookSecret" placeholder={t('email-callbacks.form.webhook.secret.placeholder')} {...inputProps} value={field.value} aria-invalid={Boolean(field.error)} />
                <Button class="flex-shrink-0" variant="outline" size="icon" onClick={() => setValue(form, 'webhookSecret', generateEmailCallbackSecret())}>
                  <div class="i-tabler-refresh size-4"></div>
                </Button>
              </div>
              {field.error && <div class="text-red-500 text-sm">{field.error}</div>}
          ...
          {fieldArray => (
            <div>
              <For each={fieldArray.items}>
                {(_, index) => (
                  <Field name={`allowedOrigins.${index()}`}>
                    {(field, props) => (
                      <TextFieldRoot class="flex flex-col gap-1 flex-1 mb-2">
                        <div class="flex gap-2 w-full mt-1">
                          ...
                          <Button variant="outline" size="icon" onClick={() => remove(form, 'allowedOrigins', { at: index() })} aria-label={t('email-callbacks.form.allowed-origins.remove-email')}>
              ...
              <Button variant="outline" onClick={() => insert(form, 'allowedOrigins', { value: '' })} class="gap-2">
      ...
      <div class="flex gap-2 mt-4 justify-between items-center px-6 sm:px-0">
        ...
        <div class="flex gap-2 flex-shrink-0">{props.submitButton({ form })}</div>
      </div>
    </Form>
  );
};
