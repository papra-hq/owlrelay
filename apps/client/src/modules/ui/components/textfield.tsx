...
import type { TextFieldDescriptionProps, TextFieldErrorMessageProps, TextFieldInputProps, TextFieldLabelProps, TextFieldRootProps } from '@kobalte/core/text-field';
...
export const textfieldLabel = cva('text-sm data-[disabled]:(cursor-not-allowed opacity-70) font-medium', {
  variants: {
    label: {
...
export function TextFieldDescription<T extends ValidComponent = 'div'>(props: PolymorphicProps<T, textFieldDescriptionProps<T>>) {
  ...
  return <TextFieldPrimitive.Description class={cn(textfieldLabel({ description: true }), local.class)} {...rest} />;
}
...
export function TextField<T extends ValidComponent = 'input'>(props: PolymorphicProps<T, textFieldInputProps<T>>) {
  ...
  return (
    <TextFieldPrimitive.Input
      class={cn(
        'flex h-9 w-full rounded-md border border-input bg-inherit px-3 py-1 text-sm file:(border-0 bg-transparent text-sm font-medium) placeholder:text-muted-foreground focus-visible:(outline-none ring-1.5 ring-ring) disabled:(cursor-not-allowed opacity-50) transition-shadow',
        local.class,
      )}
      {...rest}
    />
