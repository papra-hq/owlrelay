...
export function TableRow(props: ComponentProps<'tr'>) {
  ...
  return <tr class={cn('border-b transition-colors data-[state=selected]:bg-muted', local.class)} {...rest} />;
}

export function TableHead(props: ComponentProps<'th'>) {
  const [local, rest] = splitProps(props, ['class']);

  return <th class={cn('h-10 px-2 text-left align-middle font-medium text-muted-foreground [&:has([role=checkbox])]:pr-0 [&>[role=checkbox]]:translate-y-[2px]', local.class)} {...rest} />;
}

export function TableCell(props: ComponentProps<'td'>) {
  const [local, rest] = splitProps(props, ['class']);

  return <td class={cn('p-2 align-middle [&:has([role=checkbox])]:pr-0 [&>[role=checkbox]]:translate-y-[2px]', local.class)} {...rest} />;
}
