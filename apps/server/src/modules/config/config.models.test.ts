...
describe('config models', () => {
  describe('getPublicConfig', () => {
    test(`the public config only contains the followings info:
        ...
        - auth.showLegalLinksOnAuthPage Whether to show legal links on the auth pages
        - auth.providers.*.isEnabled Wether a oauth provider is enabled
        - emailCallbacks.availableDomains The domains that can be used for email callbacks
      ...
      const config = {
        ...
        emailCallbacks: {
          availableDomains: ['example.com', 'example.org'],
        },
