...
async function saveUserPlanId({ customerId, planId, db }: { customerId: string; planId: string; db: Database }) {
  const [user] = await db.update(usersTable).set({ planId }).where(eq(usersTable.customerId, customerId)).returning();

  return { user };
}

async function saveUserCustomerId({ customerId, userId, db }: { customerId: string; userId: string; db: Database }) {
  const [user] = await db.update(usersTable).set({ customerId }).where(eq(usersTable.id, userId)).returning();
